...
def logar():
    ...
    if login == '' or senha == '':
        ...
        return
    try:
        resultado = dbmanager.get_funcionarios()
        for check in resultado:
            if login == check[2]:
                if senha == check[4]:
                    try:
                        result = dbmanager.get_carros()
...
def logout14():
    ...
    pt14.close()
    pt11.show()
    dados_lidos = dbmanager.get_funcionarios()
...
def logout15():
    ...
    pt15.close()
    pt10.show()
    dados_lidos = dbmanager.get_clientes()
...
def chamar_tela_devkwid():
    pt41.show()
    try:
        resultado = dbmanager.get_clientes()
        for check in resultado:
            if "Kwid" == check[0]:
...
def cadastrar_cliente():
    ...
    try:
        cursor = banco.cursor()
        dbmanager.insert_cliente(nome,cnh,nasc,end,tel)
...
def editar_cliente():
    ...
    for j in caracteres:
        if j in tel_novo:
            ...
            return
    try:
        dbmanager.update_cliente(nome_novo, cnh_novo, nasc_novo, end_novo, tel_novo, nome, cnh, end, tel)
...
def editar_funcionario():
    ...
    if(senha_novo==senha2_novo):
        try:
            dbmanager.update_carro(nome_novo, login_novo, opcao, senha_novo, nome, login, cargo, senha)
...
def excluir_funcionario():
    try:
        ...
        login = pt11.tabela_funcio.item(row, 2).text()
        senha = pt11.tabela_funcio.item(row, 3).text()
        dbmanager.delete_funcionario(nome, cargo, login, senha)
...
def excluir_cliente():
    try:
        ...
        dbmanager.delete_cliente(nome,cnh)
